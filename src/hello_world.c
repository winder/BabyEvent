...
static void sync_tuple_changed_callback(const uint32_t key, const Tuple* new_tuple, const Tuple* old_tuple, void* context) {
  ...
  char *targetText = NULL;
	TextLayer *targetLayer = NULL;
	char *timeSinceText = NULL;
  TextLayer *timeSinceLayer = NULL;
  ...
  switch (key) {
    ...
    case LAST_SLEEPING:
        ...
        break;
    case LAST_BOTTLE:
        targetText = timeTextUp;
        targetLayer = bottleTextLayer;
        timeSinceText = timeSinceTextUp;
        timeSinceLayer = bottleSinceTextLayer;
        persistKey = PERSIST_BOTTLE;
    		bottleStart = new_tuple->value->uint32;
    ...
    case LAST_DIAPER:
    		targetText = timeTextMiddle;
    		targetLayer = diaperTextLayer;
        timeSinceText = timeSinceTextMiddle;
        timeSinceLayer = diaperSinceTextLayer;
        persistKey = PERSIST_DIAPER;
    		diaperStart = new_tuple->value->uint32;
  ...
  if (persistKey != -1)
    persist_write_int(persistKey, new_tuple->value->uint32);
  if (timeSinceText != '\0' && timeSinceLayer != '\0')
    setTimeSinceText(new_tuple->value->uint32, timeSinceText, timeSinceLayer);
	if (targetText != '\0' && targetLayer != '\0')
	  setTimeText(new_tuple->value->uint32, targetText, targetLayer);
}
